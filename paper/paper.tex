...
be mathematically expressed as a convolution of a system's response function with a signal. 
...
\verb|DeconvOptim.jl| works both on CPUs and GPUs and utilizes recent advancements in Julias Automatic Differentiation ecosystem. The performance on CPUs surpasses the one of existing open source libraries. 
The library is applicable to one dimensional time series datasets but also to multi-dimensional microscopy imaging datasets.
\end{abstract}

\todo{fix code formatting}
...
\end{align}
Due to the noise and the low pass filter effect of the PSF, deconvolution is an ill-posed problem which cannot be solved directly.\\
...
Wiener filter assumes additive noise which is often not true, especially in low light conditions as in microscopy or astronomy. 
...
Afterwards we calculate the gradient of the loss function with respect to reconstruction $x(\mathbf r)$ via automatic differentation (AD).
...
However, as we will see later due to the more complex optimization routine the speed-up in using L-BFGS is for simple 
deconvolution minor.
...
function total_variation(arr, ϵ=eltype(arr)(1e-8))
    @tullio r = sqrt(ϵ + 
        ...
        \end{lstlisting}
        \lstinline{ϵ} is needed for regions where \lstinline{arr == 0} because otherwise the gradient return infinite values.
...
r, o = deconvolution(measured, psf, loss=Gauss(), 
    ...
    \subsection{Influence of Regularizer}
        ...
        \begin{figure}[h]
            ...
            \begin{subfigure}[b]{.166\textwidth}
                ...
                \label{fig:fabioc}
            \end{subfigure}%            
            \caption{a) has been convolved with a PSF and affected by Poisson shot noise (1000 photons expected for the brightest pixel). b) has been deconvolved for 15 iterations without an regularizer whereas c) was regularizer with Good's roughness.}
            \label{fig:image}
        \end{figure}
    ...
    \subsection{Full Deconvolution}
        ...
        but in terms of performance the runtimes might by only by a factor of 2-4 faster than the runtimes we obtained (Ikoma et al. used 140 iterations in their datasets).
