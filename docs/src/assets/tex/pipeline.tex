...
\begin{document}
    ...
    every node/.style={fill=white, font=\sffamily}, align=center]
        ...
        \node[block] (start) {rec \\(variables to optimize for)};
        \node[block, below of=start] (map) {Mapping \\ \footnotesize (sets constraints like positivity)};
        ...
        \path [line] (tloss) -- (opt);
        \path [line] (reg) --node [midway] {Weighting with $\lambda \cdot$} (tloss) ;

            
        \path[line] (opt) --++ (5, 0) --++ (0, 10.2) node[] {AD calculates gradient \\ to optimize for rec} -- (start) ;
